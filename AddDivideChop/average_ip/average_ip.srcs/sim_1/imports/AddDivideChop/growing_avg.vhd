...
entity growing_avg is 
    generic(         N : integer := 16;
              MAX_AVGS : integer := 2;  -- must be base 2
...
architecture behv of growing_avg is 
    ...
    signal lost_sum  : unsigned(SUM_WIDTH-1 downto 0);
...
begin 
    ...
    begin 
        if (clk'event and clk='1') then  
            if (AVGS < 2**MAX_AVGS -1) then
                if (AVGS = 0) then
                    sum <= lost_sum + unsigned((SUM_WIDTH-1 downto x'length => '0') & x);
                else 
                    sum <= sum + unsigned((SUM_WIDTH-1 downto x'length => '0') & x); 
                end if;        
                 -- sum & lost_sum out stays fixed
                 AVGS <= AVGS + 1;
            else 
                sum_out <= sum;
                lost_sum <= unsigned((SUM_WIDTH-1 downto x'length => '0') & x);
                -- sum stays fixed
                AVGS <= (others => '0');          
            end if;
        end if;
